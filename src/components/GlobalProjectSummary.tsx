...
export default function GlobalProjectSummary({ meta, colors, deliveryColorOptions = [], groupedOrders = [], darkMode = false }: Props) {
  ...
  return <div className={`sticky top-0 z-40 border-b shadow-md ${darkMode ? 'border-violet-300/20 bg-[#111827]/[.97] text-white' : 'border-violet-200 bg-white/[.98] text-slate-900'} backdrop-blur`}>
    <div className="mx-auto flex max-w-[1800px] items-center gap-2 overflow-x-auto px-3 py-1.5">
      <div className="flex min-w-0 flex-1 items-center gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet-700 text-white"><ClipboardList size={16}/></div><div className="min-w-0"><p className="truncate text-[9px] font-black uppercase tracking-[.16em] text-violet-700">Projet actif</p><div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 text-[11px] font-bold"><span>Order # <b className="font-mono">{meta?.order || '—'}</b></span><span>PO <b className="font-mono">{meta?.po || '—'}</b></span><span className="max-w-[220px] truncate">{meta?.customer || 'Client non sélectionné'}</span>{groupedOrders.length > 0 && <span className="rounded bg-violet-100 px-1.5 py-0.5 text-[9px] font-black text-violet-800">{groupedOrders.length + 1} commandes</span>}</div></div></div>
      ...
      <button type="button" onClick={() => setExpanded(value => !value)} className="flex items-center gap-1 rounded-lg border border-violet-200 bg-violet-50 px-2.5 py-1.5 text-[10px] font-black text-violet-700"><BarChart3 size={14}/><span>{rows.length} couleur(s)</span>{expanded ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}</button>
    </div>
