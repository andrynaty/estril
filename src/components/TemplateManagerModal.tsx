...
import { Box, Edit3, Plus, Ruler, Trash2, Weight, X } from 'lucide-react';

type Category = 'dimension' | 'weight_piece' | 'weight_carton';
...
const categoryLabels: Record<Category, string> = { dimension: '📐 DIM. CARTON', weight_piece: '⚖️ POIDS PIÈCE', weight_carton: '📦 POIDS CARTON' };
...
export default function TemplateManagerModal({ isOpen, onClose, darkMode = false }: Props) {
  ...
  const save = async () => {
    ...
    if (category === 'dimension' && [form.length_cm, form.width_cm, form.height_cm].some(value => !Number(value) || Number(value) <= 0)) { setMessage('Longueur, largeur et hauteur doivent être supérieures à zéro.'); return; }
    if (category !== 'dimension' && (!Number(form.weight_kg) || Number(form.weight_kg) <= 0)) { setMessage('Le poids doit être supérieur à zéro.'); return; }
  ...
  return <div className="fixed inset-0 z-[9500] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm"><div className={`flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border shadow-2xl ${darkMode ? 'border-slate-700 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-900'}`}>
    ...
    <div className="flex flex-wrap gap-2 border-b border-slate-200 p-4">{(Object.keys(categoryLabels) as Category[]).map(key => <button key={key} onClick={() => setCategory(key)} className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-black ${category === key ? 'bg-teal-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{key === 'dimension' ? <Ruler size={14}/> : key === 'weight_piece' ? <Weight size={14}/> : <Box size={14}/>} {categoryLabels[key]}</button>)}</div>
    <div className="grid min-h-0 flex-1 gap-4 overflow-auto p-5 lg:grid-cols-[1fr_330px]">
      <div className="overflow-auto rounded-xl border border-slate-200"><table className="w-full text-left text-sm"><thead className="bg-slate-900 text-xs uppercase text-white"><tr><th className="px-4 py-3">Nom</th>{category === 'dimension' ? <><th className="px-4 py-3">Longueur</th><th className="px-4 py-3">Largeur</th><th className="px-4 py-3">Hauteur</th><th className="px-4 py-3">Volume m³</th></> : <th className="px-4 py-3">Poids kg</th>}<th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody>{rows.map(row => <tr key={row.id} className="border-t border-slate-100"><td className="px-4 py-3 font-bold">{row.name}</td>{category === 'dimension' ? <><td className="px-4 py-3">{row.length_cm}</td><td className="px-4 py-3">{row.width_cm}</td><td className="px-4 py-3">{row.height_cm}</td><td className="px-4 py-3">{((Number(row.length_cm || 0) * Number(row.width_cm || 0) * Number(row.height_cm || 0)) / 1000000).toFixed(4)}</td></> : <td className="px-4 py-3">{row.weight_kg}</td>}<td className="px-4 py-3"><div className="flex justify-end gap-2"><button onClick={() => openEdit(row)} className="rounded-lg border p-2 text-teal-700"><Edit3 size={14}/></button><button onClick={() => remove(row)} className="rounded-lg border border-rose-200 p-2 text-rose-600"><Trash2 size={14}/></button></div></td></tr>)}{rows.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-sm text-slate-500">Aucun gabarit dans cette catégorie.</td></tr>}</tbody></table></div>
      <aside className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between"><h3 className="font-black">{editing ? 'Modifier le gabarit' : 'Nouveau gabarit'}</h3><button onClick={openNew} className="flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold"><Plus size={13}/> Nouveau</button></div><label className="mt-4 block text-xs font-bold">Nom<input value={String(form.name || '')} onChange={e => setForm({ ...form, name: e.target.value })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2 text-sm" placeholder="STANDARD"/></label>{category === 'dimension' ? <div className="mt-3 grid grid-cols-3 gap-2">{([['length_cm','Longueur'],['width_cm','Largeur'],['height_cm','Hauteur']] as const).map(([key, label]) => <label key={key} className="text-xs font-bold">{label}<input type="number" value={form[key] == null ? '' : form[key]} onChange={e => setForm({ ...form, [key]: Number(e.target.value) })} className="mt-1 w-full rounded-lg border bg-white px-2 py-2 text-sm"/></label>)}</div> : <label className="mt-3 block text-xs font-bold">Poids (kg)<input type="number" value={form.weight_kg == null ? '' : form.weight_kg} onChange={e => setForm({ ...form, weight_kg: Number(e.target.value) })} className="mt-1 w-full rounded-lg border bg-white px-3 py-2 text-sm"/></label>}{category === 'dimension' && <p className="mt-3 rounded-lg bg-white p-3 text-xs text-slate-600">Volume calculé : <strong>{volume.toFixed(4)} m³</strong></p>}<button onClick={save} className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-teal-700 px-4 py-2 text-xs font-black text-white"><Plus size={14}/>{editing ? 'ENREGISTRER LA MODIFICATION' : 'AJOUTER LE GABARIT'}</button>{message && <p className="mt-3 rounded-lg bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">{message}</p>}</aside>
    </div>
  </div></div>;
}
