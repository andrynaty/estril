...
export default function IndustrialCenter({ onBack, onLoadProject, projectPayload, initialTab = 'dashboard', showTabs = true }: { onBack: () => void; onLoadProject?: (project: any) => Promise<void> | void; projectPayload?: any; initialTab?: CenterTab; showTabs?: boolean }) {
  ...
  const breakdownPercent = breakdownPoTotal > 0 ? Math.min(100, (breakdownPlTotal / breakdownPoTotal) * 100) : 0;
  const breakdownByColorView = breakdownByColor.map(item => {
    ...
    return { ...item, plQty, poQty, percent: poQty > 0 ? Math.min(100, (plQty / poQty) * 100) : 0 };
  });
  ...
  return <section className="flex-1 min-h-0 overflow-auto rounded-2xl border border-slate-200 bg-white shadow-sm" style={{ ['--ruba-accent' as any]: accent }}>
    ...
    <aside className="mx-5 mt-4 w-auto rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 via-white to-teal-50 p-4 text-slate-800 shadow-sm xl:fixed xl:right-5 xl:top-24 xl:z-30 xl:mx-0 xl:w-72">
      ...
      <div className="mt-4 border-t border-indigo-200 pt-3"><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase text-indigo-700">Breakdown vs PO QTY</span><b className="font-mono text-indigo-800">{breakdownPercent.toFixed(1)}%</b></div><div className="mt-2 h-2 w-full rounded-full bg-indigo-100"><div className="h-2 rounded-full bg-indigo-600" style={{ width: `${breakdownPercent}%` }} /></div><p className="mt-1 text-[10px] text-slate-500">{breakdownPlTotal.toLocaleString('fr-FR')} / {breakdownPoTotal.toLocaleString('fr-FR')} pcs</p>{breakdownByColorView.length > 0 && <div className="mt-3 space-y-2">{breakdownByColorView.map(item => <div key={item.color}><div className="flex justify-between gap-2 text-[10px]"><span className="max-w-[120px] truncate font-semibold">{item.color}</span><span className="font-mono">{item.percent.toFixed(1)}%</span></div><div className="mt-1 h-1.5 w-full rounded-full bg-slate-200"><div className="h-1.5 rounded-full bg-teal-600" style={{ width: `${item.percent}%` }} /></div><p className="mt-0.5 text-[9px] text-slate-500">PL {item.plQty.toLocaleString('fr-FR')} / PO {item.poQty.toLocaleString('fr-FR')}</p></div>)}</div>}</div>
    </aside>
