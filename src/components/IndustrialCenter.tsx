...
export default function IndustrialCenter({ onBack, onLoadProject, projectPayload, initialTab = 'dashboard', showTabs = true }: { onBack: () => void; onLoadProject?: (project: any) => Promise<void> | void; projectPayload?: any; initialTab?: CenterTab; showTabs?: boolean }) {
  ...
  const [csvStatus, setCsvStatus] = useState<{ found: boolean; source?: string; modifiedAt?: string; rows?: number; candidates?: string[] }>({ found: false });
  ...
  useEffect(() => {
    ...
    window.rubaDesktop.listDeliveryPlans(selectedProjectId).then((nextPlans) => {
      ...
      if (Array.isArray(payload.rows)) setRows(payload.rows.map(normalizeRow));
  ...
  const refresh = async (query = deferredSearch) => {
    ...
    const [nextSummary, nextProjects, nextFiles, root] = await Promise.all([api.dbSummary(), api.listProjects(query), api.listFiles({ search: query }), api.getStorageRoot()]);
  ...
  useEffect(() => { refresh().catch(() => setMessage('Le centre local sera disponible dans la version desktop.')); }, [deferredSearch]);
