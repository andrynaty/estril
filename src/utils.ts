...
export const BG_COLORS_DARK = [
  'rgba(79,142,247,0.22)', 'rgba(56,217,169,0.22)', 'rgba(245,166,35,0.22)', 'rgba(155,114,245,0.22)',
  'rgba(224,92,92,0.22)', 'rgba(6,182,212,0.22)', 'rgba(236,72,153,0.22)', 'rgba(246,224,94,0.22)'
];

export const BG_COLORS_LIGHT = [
  'rgba(79,142,247,0.20)', 'rgba(56,217,169,0.20)', 'rgba(245,166,35,0.20)', 'rgba(155,114,245,0.20)',
  'rgba(224,92,92,0.20)', 'rgba(6,182,212,0.20)', 'rgba(236,72,153,0.20)', 'rgba(246,224,94,0.20)'
];
...
export function getRemainderRowColor(
  ...
  const palette = [
    { bg: '#ffea00', text: '#000000', darkBg: '#d97706', darkText: '#ffffff' }, // Vivid Electric Yellow
    { bg: '#22c55e', text: '#ffffff', darkBg: '#15803d', darkText: '#ffffff' }, // Vivid Green
    { bg: '#00e5ff', text: '#000000', darkBg: '#0284c7', darkText: '#ffffff' }, // Vivid Electric Cyan
    { bg: '#ff2a85', text: '#ffffff', darkBg: '#be185d', darkText: '#ffffff' }, // Vivid Hot Pink
    { bg: '#a855f7', text: '#ffffff', darkBg: '#7e22ce', darkText: '#ffffff' }, // Vivid Purple
    { bg: '#ff7800', text: '#000000', darkBg: '#ea580c', darkText: '#ffffff' }, // Vivid Orange
    { bg: '#00f5d4', text: '#000000', darkBg: '#0f766e', darkText: '#ffffff' }, // Vivid Turquoise
    { bg: '#cddc39', text: '#000000', darkBg: '#65a30d', darkText: '#ffffff' }  // Vivid Lime
  ];
  
  const c = palette[index % palette.length];
  return {
    bg: darkMode ? c.darkBg : c.bg,
    text: darkMode ? c.darkText : c.text,
    border: darkMode ? 'border-amber-500/30' : 'border-slate-900',
    rawBg: c.bg,
    rawText: c.text
  };
}
...
export function computeColorResult(
  ...
  if (activeMode === 'mixte_autorise' && colorConfig.customRemaindersEnabled && colorConfig.customRemainders && colorConfig.customRemainders.length > 0) {
    ...
    colorConfig.customRemainders.forEach(cc => {
      ...
      Object.entries(cc.sizes).forEach(([sz, qte]) => {
        if (qte > 0 && tailles.includes(sz)) {
          hasItems = true;
          mixedSizes[sz] = qte;
          boxSum += qte;
          const spec = sizes[sz];
    ...
    // Pack any leftovers (unallocated remainders) as solid_r (individual solid remainder cartons)
    tailles.forEach(t => {
      const q = sizes[t]?.qtyTot || 0;
      const cap = sizes[t]?.cap || 25;
      const r = q % cap;

      let allocatedInCustom = 0;
      colorConfig.customRemainders?.forEach(cc => {
        allocatedInCustom += cc.sizes[t] || 0;
      });

      const leftover = r - allocatedInCustom;
      if (leftover > 0) {
